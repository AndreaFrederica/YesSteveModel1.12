...
public class HashMapStruct implements Struct {
    ...
    public Struct copy() {
        if (this.isRightValue) {
            return new HashMapStruct(this.properties);
        } else {
            return new HashMapStruct(new PooledStringHashMap<>(this.properties));
        }
    }
    ...
    public String toString() {
        ...
        boolean first = true;
        for (Int2ReferenceMap.Entry<Object> entry : this.properties.int2ReferenceEntrySet()) {
            if (!first) {
                builder.append(", ");
            }
            first = false;
