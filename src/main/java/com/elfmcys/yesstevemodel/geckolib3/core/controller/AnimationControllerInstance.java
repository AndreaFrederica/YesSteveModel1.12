...
public class AnimationControllerInstance {
    private final Int2ReferenceOpenHashMap<NewBoneAnimationQueue> boneAnimationQueues;

    private final ReferenceArrayList<NewBoneAnimationQueue> activeBoneAnimationQueues;
    ...
    private com.elfmcys.yesstevemodel.geckolib3.core.AnimationState animationState;
    ...
    public AnimationControllerInstance(AnimatableEntity<?> animatable, float transitionLengthTicks, boolean isScaleTransitionSpecial) {
        ...
        this.animationState = com.elfmcys.yesstevemodel.geckolib3.core.AnimationState.IDLE;
    ...
    public void initBoneQueues(List<BoneTopLevelSnapshot> list) {
        ...
        for (BoneTopLevelSnapshot boneTopLevelSnapshot : list) {
            this.boneAnimationQueues.put(boneTopLevelSnapshot.bone.getBoneId(), new NewBoneAnimationQueue(boneTopLevelSnapshot));
        }
    }
    ...
    public void setAnimation(@Nullable String animationName, @Nullable ILoopType loopType) {
        ...
        if (this.lastRequestedAnimation != null && this.lastRequestedAnimation.getRight().equals(animationName) && this.lastRequestedAnimation.getLeft() == loopType) {
            return;
        }
        clearAnimation();
        this.lastRequestedAnimation = Pair.of(loopType, animationName);
        Animation animation = this.animatable.getAnimation(animationName);
        if (animation == null) {
            return;
        }
        this.pendingAnimation = Pair.of(loopType != null ? loopType : animation.loop, animation);
    }
    ...
    public void process(float tick, ExpressionEvaluator<AnimationContext<?>> evaluator, boolean z) {
        ...
        if (this.animationState == com.elfmcys.yesstevemodel.geckolib3.core.AnimationState.ENDING_TRANSITION && adjustedTick >= defaultTransitionTick) {
            clearAnimation();
        }
        if (this.animationState == com.elfmcys.yesstevemodel.geckolib3.core.AnimationState.RUNNING && this.currentAnimationLoop == ILoopType.EDefaultLoopTypes.PLAY_ONCE && adjustedTick >= this.currentAnimation.animationLength) {
        ...
        if (this.animationState == com.elfmcys.yesstevemodel.geckolib3.core.AnimationState.IDLE) {
            this.context.executeRenderLayers(evaluator);
            ...
            if (this.transitionInterpolator.getProgress() > 0.0f) {
                this.animationState = com.elfmcys.yesstevemodel.geckolib3.core.AnimationState.BEGINNING_TRANSITION;
            } else {
                this.animationState = com.elfmcys.yesstevemodel.geckolib3.core.AnimationState.RUNNING;
            }
        }
        resetAllQueues();
        if (this.animationState == com.elfmcys.yesstevemodel.geckolib3.core.AnimationState.BEGINNING_TRANSITION) {
        ...
        if (this.animationState == com.elfmcys.yesstevemodel.geckolib3.core.AnimationState.RUNNING) {
        ...
        if (this.animationState == com.elfmcys.yesstevemodel.geckolib3.core.AnimationState.ENDING_TRANSITION) {
    ...
    private void startEndingTransition(float tick) {
        if (this.animationState == com.elfmcys.yesstevemodel.geckolib3.core.AnimationState.RUNNING || this.animationState == com.elfmcys.yesstevemodel.geckolib3.core.AnimationState.BEGINNING_TRANSITION) {
            float adjustedTick = adjustTick(tick);
            for (NewBoneAnimationQueue animationQueue : this.activeBoneAnimationQueues) {
            ...
            this.isAnimationFinished = true;
            this.animationState = com.elfmcys.yesstevemodel.geckolib3.core.AnimationState.ENDING_TRANSITION;
        }
    }
    ...
    private void processBeginningTransition(ExpressionEvaluator<AnimationContext<?>> evaluator, float tick) {
        ...
        for (NewBoneAnimationQueue boneAnimationQueue : this.activeBoneAnimationQueues) {
    ...
    public void resetAllQueues() {
        if (this.animationState != com.elfmcys.yesstevemodel.geckolib3.core.AnimationState.IDLE) {
            for (NewBoneAnimationQueue activeBoneAnimationQueue : this.activeBoneAnimationQueues) {
    ...
    private boolean applyPendingAnimation() {
        ...
        this.pendingAnimation = null;
        this.currentAnimation = pair.getRight();
        this.currentAnimationLoop = pair.getLeft();
        this.isAnimationFinished = false;
        for (BoneAnimation animation : this.currentAnimation.boneAnimations) {
            NewBoneAnimationQueue queue = this.boneAnimationQueues.get(animation.boneId);
            if (queue != null) {
    ...
    public com.elfmcys.yesstevemodel.geckolib3.core.AnimationState getAnimationState() {
        return this.animationState;
    }

    public ReferenceArrayList<NewBoneAnimationQueue> getActiveBoneAnimationQueues() {
