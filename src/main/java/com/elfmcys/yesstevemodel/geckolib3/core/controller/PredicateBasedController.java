...
public class PredicateBasedController<T extends AnimatableEntity<?>> implements IAnimationController<T> {
    ...
    public void process(AnimationEvent<T> event, ExpressionEvaluator<AnimationContext<?>> evaluator, boolean isMoving) {
        ...
        if (playState == PlayState.CONTINUE) {
            this.transitionInterpolator.process((float) event.animationTick, evaluator, isMoving);
            ...
            if (state == AnimationState.BEGINNING_TRANSITION || state == AnimationState.RUNNING) {
                this.transitionInterpolator.beginEndingTransition((float) event.animationTick);
            ...
            this.transitionInterpolator.process((float) event.animationTick, evaluator, false);
            this.transitionInterpolator.resetAllQueues();
            this.needsReset = true;
        }
        event.getAnimatableEntity().setAnimationState(this.name, this.transitionInterpolator.getAnimationState());
    }
    ...
    public com.elfmcys.yesstevemodel.geckolib3.core.builder.Animation getCurrentAnimation() {
        return this.transitionInterpolator.getCurrentAnimation();
    }

    @Override
    public void setAnimation(String animationName, @Nullable ILoopType loopType) {
        this.transitionInterpolator.setAnimation(animationName, loopType);
    }
    ...
    public void forEachTransform(Consumer<BoneTransformProvider> consumer) {
        if (!this.needsReset) {
            for (NewBoneAnimationQueue boneAnimationQueue : this.transitionInterpolator.getActiveBoneAnimationQueues()) {
    ...
    private static class TransformProviderRecord implements BoneTransformProvider {
        private final NewBoneAnimationQueue data;

        TransformProviderRecord(NewBoneAnimationQueue data) {
            this.data = data;
        }
        ...
        public Optional<TransitionVector3f> getRotation(ExpressionEvaluator<AnimationContext<?>> evaluator) {
            ...
            if (point instanceof ConstantPoint) {
                ...
                result.setPercentCompleted((float) point.getPercentCompleted());
                float blendWeight = this.data.getBlendWeight();
                if (blendWeight != 1.0f) {
                    result.x *= blendWeight;
                    result.y *= blendWeight;
                    result.z *= blendWeight;
                }
            } else if (point instanceof TransitionPoint transition) {
                Vector3f vector3fMul = transition.evaluateRaw(evaluator);
                float bw = this.data.getBlendWeight();
                vector3fMul.x *= bw;
                vector3fMul.y *= bw;
                vector3fMul.z *= bw;
                ...
                if (blendWeight2 != 1.0f) {
                    result.x *= blendWeight2;
                    result.y *= blendWeight2;
                    result.z *= blendWeight2;
                }
                result.setPercentCompleted(0.0f);
            }
