...
public class YSMFolderDeserializer implements AutoCloseable {
    ...
    private byte[] readResource(String relativePath) {
        ...
        try {
            ...
            if (inMemoryFiles == null) {
                ...
                data = Files.exists(target) && Files.isRegularFile(target) ? Files.readAllBytes(target) : null;
            } else {
    ...
    public RawYsmModel deserialize() {
        ...
        if (ysmJsonBytes != null) {
            ...
            JsonObject ysmJson = new JsonParser().parse(jsonStr).getAsJsonObject();
    ...
    private void parseYsmJson(JsonObject ysmJson) {
        if (ysmJson.has("metadata")) {
            parseMetadata(ysmJson.getAsJsonObject("metadata"));
        }
        if (ysmJson.has("properties")) {
        ...
        if (ysmJson.has("files")) {
            ...
            if (files.has("vehicles")) {
                parseSubEntities(files.get("vehicles"), model.vehicles, "vehicle");
            }
            if (files.has("projectiles")) {
                parseSubEntities(files.get("projectiles"), model.projectiles, "projectile");
            }
        }
    }
    ...
    private void parseMainEntity(JsonObject playerObj) {
        if (playerObj.has("model") && playerObj.get("model").isJsonObject()) {
            ...
            if (modelObj.has("main")) {
                ...
                if (geoData != null) {
                    model.mainEntity.mainModel = parseGeometry(geoData, 1);
                }
            }
            if (modelObj.has("arm")) {
                byte[] geoData = readResource(modelObj.get("arm").getAsString());
                if (geoData != null) {
                    model.mainEntity.armModel = parseGeometry(geoData, 2);
                }
            }
        }
    ...
    private void parseSubEntities(JsonElement sectionElem, Map<String, RawYsmModel.RawSubEntity> targetMap, String defaultIdentifier) {
        ...
        for (JsonObject item : items) {
            ...
            if (item.has("match")) {
                ...
                if (match.isJsonArray()) {
                    ...
                    for (int i = 0; i < mArr.size(); i++) {
                        sub.matchIds[i] = mArr.get(i).getAsString();
                    }
            ...
            if (item.has("model")) {
                ...
                if (geoData != null) {
                    sub.model = parseGeometry(geoData, 3);
                }
            }
        ...
        if (geoObj.has("description")) {
            ...
            if (desc.has("visible_bounds_offset") && desc.get("visible_bounds_offset").isJsonArray()) {
                ...
                for (int i = 0; i < offsetArr.size(); i++) {
                    geo.visibleBoundsOffset[i] = offsetArr.get(i).getAsFloat();
                }
            } else {
        ...
        for (Map.Entry<String, JsonElement> entry : sorted) {
            ...
            if (valElem.isJsonObject()) {
                ...
                if (obj.has("lerp_mode")) {
                    ...
                    if ("catmullrom".equals(lm)) {
                        kf.interpolationMode = 2;
                    } else if ("step".equals(lm)) {
    ...
    private void parseAnimationControllers(byte[] data, Map<String, RawYsmModel.RawAnimationController> targetMap) {
        ...
        for (Map.Entry<String, JsonElement> acEntry : acs.entrySet()) {
            if (!acEntry.getValue().isJsonObject()) {
                continue;
            }
            ...
            if (acObj.has("states") && acObj.get("states").isJsonObject()) {
                ...
                for (Map.Entry<String, JsonElement> sEntry : statesObj.entrySet()) {
                    if (!sEntry.getValue().isJsonObject()) {
                        continue;
                    }
                    ...
                    if (sObj.has("on_entry") && sObj.get("on_entry").isJsonArray()) {
                        for (JsonElement oe : sObj.getAsJsonArray("on_entry")) {
                            state.onEntry.add(oe.getAsString());
                        }
                    }

                    if (sObj.has("on_exit") && sObj.get("on_exit").isJsonArray()) {
                        for (JsonElement oe : sObj.getAsJsonArray("on_exit")) {
                            state.onExit.add(oe.getAsString());
                        }
                    }
                    ...
                    if (sObj.has("sound_effects") && sObj.get("sound_effects").isJsonArray()) {
                        for (JsonElement se : sObj.getAsJsonArray("sound_effects")) {
                            if (se.isJsonObject()) {
                                state.soundEffects.add(getStr(se.getAsJsonObject(), "effect", ""));
                            } else if (se.isJsonPrimitive()) {
                                state.soundEffects.add(se.getAsString());
                            }
                        }
                    }
    ...
    private void processGlobalResourceFile(String relativePath, byte[] data) {
        if (relativePath.startsWith("sounds/") || relativePath.endsWith(".ogg")) {
            ...
            try {
                ...
                JsonObject langJson = new JsonParser().parse(langJsonStr).getAsJsonObject();
    ...
    private static ImageMeta parseImageMeta(byte[] data, String path) {
        ...
        try {
            ...
            switch (format) {
                case 1, 3, 4, 5 -> img = ImageIO.read(new ByteArrayInputStream(data));
                default -> {
                }
            }
            if (img != null) {
    ...
    public static int detectFormat(byte[] data) {
        ...
        if (data.length >= 12 && data[0] == 'R' && data[1] == 'I' && data[2] == 'F' && data[3] == 'F' && data[8] == 'W' && data[9] == 'E' && data[10] == 'B' && data[11] == 'P') {
            return 4;
        }
    ...
    private void parseLegacyFormat() {
        ...
        if (infoData != null) {
            try {
                JsonObject infoObj = new JsonParser().parse(new String(infoData, StandardCharsets.UTF_8)).getAsJsonObject();
    ...
    private void parseLegacyMetadata(JsonObject infoObj, boolean overwrite) {
        ...
        if (infoObj.has("free") && (overwrite || !model.properties.isFree)) {
            model.properties.isFree = getBool(infoObj, "free", false);
        }

        if (infoObj.has("authors") && infoObj.get("authors").isJsonArray() && (overwrite || model.metadata.authors.isEmpty())) {
        ...
        if (infoObj.has("extra_animation_names") && infoObj.get("extra_animation_names").isJsonArray() && (overwrite || model.properties.extraAnimations.isEmpty())) {
