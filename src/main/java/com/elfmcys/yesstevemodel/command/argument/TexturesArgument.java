...
public class TexturesArgument implements ArgumentType<String> {
    ...
    public static String getTexture(CommandContext<CommandSource> context, String name) {
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> source, SuggestionsBuilder builder) {
        if (source.getSource() instanceof ISuggestionProvider) {
            String modelName = ModelsArgument.getModel((CommandContext<CommandSource>) source, "model_id");
            if (FMLEnvironment.dist == Dist.DEDICATED_SERVER) {
                if (ServerModelManager.CACHE_NAME_INFO.containsKey(modelName)) {
                    ...
                    return ISuggestionProvider.suggest(textures, builder);
                }
            } else {
