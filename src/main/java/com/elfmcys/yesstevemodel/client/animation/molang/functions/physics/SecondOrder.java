...
public class SecondOrder implements IPhysics {
    ...
    public SecondOrder(float input, float frequency, float coefficient, float response) {
        this.arg0 = input;
        this.arg1 = MathHelper.clamp(frequency, 0, 5);
        this.arg2 = MathHelper.clamp(coefficient, 0, 1);
        this.arg3 = response;
    }
    ...
    public void update(float timeStep) {
        ...
        float frequency = MathHelper.clamp(this.arg1, 0, 5);
        float coefficient = MathHelper.clamp(this.arg2, 0, 1);
        float response = this.arg3;

        float k1 = coefficient / PI / frequency;
        float k2 = 1 / (2 * PI * frequency) / (2 * PI * frequency);
        float k3 = response * coefficient / 2 / PI / frequency;
