...
public class AnimationRegister {
    ...
    public static void setParserValue(AnimationEvent<CustomPlayerEntity> animationEvent, MolangParser parser, EntityModelData data, PlayerEntity player) {
        ...
        parser.setValue("query.body_x_rotation", () -> player.xRot);
        parser.setValue("query.body_y_rotation", () -> MathHelper.wrapDegrees(player.yRot));
        ...
        parser.setValue("query.is_eating", () -> MolangUtils.booleanToFloat(player.getUseItem().getUseAnimation() == UseAction.EAT));
        ...
        parser.setValue("query.is_jumping", () -> MolangUtils.booleanToFloat(!player.abilities.flying && !player.isPassenger() && !player.isOnGround() && !player.isInWater()));
        ...
        parser.setValue("query.is_sneaking", () -> MolangUtils.booleanToFloat(player.isOnGround() && player.getPose() == Pose.CROUCHING));
        ...
        parser.setValue("ysm.has_elytra", () -> MolangUtils.booleanToFloat(player.getItemBySlot(EquipmentSlotType.CHEST).getItem() == Items.ELYTRA));
        parser.setValue("ysm.elytra_rot_x", () -> {
            if (player instanceof ClientPlayerEntity) {
                return Math.toDegrees(((ClientPlayerEntity) player).elytraRotX);
            }
            return 0;
        });
        parser.setValue("ysm.elytra_rot_y", () -> {
            if (player instanceof ClientPlayerEntity) {
                return Math.toDegrees(((ClientPlayerEntity) player).elytraRotY);
            }
            return 0;
        });
        parser.setValue("ysm.elytra_rot_z", () -> {
            if (player instanceof ClientPlayerEntity) {
                return Math.toDegrees(((ClientPlayerEntity) player).elytraRotZ);
            }
            return 0;
        });
    ...
    private static int getEquipmentCount(PlayerEntity player) {
        int count = 0;
    ...
    private static double getMaxUseDuration(PlayerEntity player) {
