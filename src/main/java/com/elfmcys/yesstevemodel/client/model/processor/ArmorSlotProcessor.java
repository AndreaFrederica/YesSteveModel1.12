...
public class ArmorSlotProcessor<T extends GeoEntity<?>> implements ModelProcessor<T, PlayerModelBundle> {
    ...
    private final TriFunction<String, T, EntityEquipmentSlot, com.elfmcys.yesstevemodel.geckolib3.core.controller.IAnimationController<T>> controllerFactory;

    public ArmorSlotProcessor(String prefix, String category, AnimationDataProvider<PlayerModelBundle> animationDataProvider, TriFunction<String, T, EntityEquipmentSlot, com.elfmcys.yesstevemodel.geckolib3.core.controller.IAnimationController<T>> controllerFactory) {
        this.prefix = prefix;
    ...
    public ControllerFactory<T> process(PlayerModelBundle modelBundle, ModelResourceBundle resourceBundle) {
        ArrayList<Pair<String, EntityEquipmentSlot>> matchingSlots = new ArrayList<>();
        ...
        for (EntityEquipmentSlot slot : EntityEquipmentSlot.values()) {
            ...
            if (animationEntries.containsKey(slotKey)) {
                ...
                matchingSlots.add(Pair.of(slotKey, slot));
            } else if (slot.getSlotType() == EntityEquipmentSlot.Type.ARMOR && (armorCondition.hasFilter(slot) || animations.containsKey(slot.getName() + ":default"))) {
                matchingSlots.add(Pair.of(slotKey, slot));
            }
        }
        return (entity, consumer) -> {
            if (!(entity instanceof IPreviewAnimatable)) {
                for (Pair<String, EntityEquipmentSlot> pair : matchingSlots) {
