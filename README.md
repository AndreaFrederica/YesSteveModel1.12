...
[旧版Yes Steve Model 模组](https://github.com/YesSteveModel/LgeacyYSM)的 Minecraft 1.12.2 完整移植。
...
- 模型渲染仍存在问题，在非第一人称时部分透明，包括 Geo 模型和鞘翅 (https://github.com/kappa-maintainer/geckolib/issues/2)
- 手持物品渲染存在 GL 状态污染，会使背包部分格子变黑
- 模型状态似乎有污染问题，导致有时模型块变换有误
- 视角旋转过快时，头部角度会跳变
- 玩家一部分姿态（如睡觉）不对
- 模型预览界面床不会渲染，花和草光照不对，且部分动画预览玩家角度不对
- 箭矢动画似乎不应出现在模型预览界面
- 部分文字颜色不对，这是因为新版文本组件会自动重置颜色，旧版 I18n 处理 String 时不会
- 网络组件未经严格测试，在单人模式下基本正常
